...
class scopeEmulator:
    ...
    def leftJumpScanHalfSearch(self, y, mean, std):
        ...
        while( indxleft+1 < indxright ):
            mid = int(np.floor((indxright+indxleft)/2))
            dev = abs(mean-y[mid])
    ...
    def leftJumpScanEff(self,x,y):
        nmax = 100
        stat=np.array(y[0:nmax], dtype =np.float32)
        std = stat.std()
        mean = stat.mean()
        index = np.where(abs(y-mean))
...
#        return self.runTimeWalk(self, 0, self.nevent, 100,  method)
    ...
    def debug_cft(self, i, nch, control):
        points = self.getEventAdjusted(i)
        n0 = self.leftJumpScan(points[0], points[nch])
        fcft = self.simulateCFT(points[0], points[nch])
        inte = self.crossRegion(fcft,n0)
        ntrun = np.floor(100/self.dt)
        plt.plot(points[0][inte[0]-ntrun:inte[1]+ntrun], points[2][inte[0]-ntrun:inte[1]+ntrun])
        plt.plot(points[0][inte[0]-ntrun:inte[1]+ntrun], fcft(evt1[0][inte[0]-ntrun:inte[1]+ntrun]))
        res = se.getCFTiming(ievt,method = control['method'])
